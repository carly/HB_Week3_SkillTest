...
def print_indeces(my_list):
    ...
    for item in index_list:
        combined_list.append((str(item)) + " " + (str(item_list[index])))
        index += 1
...
def average(number_list):
    ...
    average = float(sum_nums) / float(len(number_list))
...
#     # """
# started to attempt... want to submit before too late. will keep trying! - Carly


    # new_string = ('')
    # index = 0
    # sentence = len(list_of_words[:])
    # answer = None
    ...
    # if len(list_of_words) == list_of_words[0]: 
    #     for word in list_of_words:
    #         new_string = ''
    #         for letter in word: 
    ...
    #     while char[index] <= (len(new_string) - 1)
    #         char += char
